...
class TestCompliance:
    ...
    def test_create_agent_task(self, url):
        response = requests.post(f"{url}/ap/v1/agent/tasks", json=self.task_data, timeout=60)
        assert response.status_code == 200
        assert Task(**response.json()).task_id

    def test_list_agent_tasks_ids(self, url):
        response = requests.get(f"{url}/ap/v1/agent/tasks", timeout=60)
        assert response.status_code == 200
    ...
    def test_get_agent_task(self, url):
        ...
        response = requests.get(f"{url}/ap/v1/agent/tasks/{task_id}", timeout=60)
        assert response.status_code == 200
    ...
    def test_list_agent_task_steps(self, url):
        ...
        response = requests.post(f"{url}/ap/v1/agent/tasks", json=self.task_data, timeout=60)
        task_id = response.json()["task_id"]
        response = requests.get(f"{url}/ap/v1/agent/tasks/{task_id}/steps", timeout=60)
        assert response.status_code == 200
        assert isinstance(response.json(), list)
    ...
    def test_list_artifacts(self, url):
        response = requests.post(f"{url}/ap/v1/agent/tasks", json=self.task_data, timeout=60)
        task_id = response.json()["task_id"]
        response = requests.get(f"{url}/ap/v1/agent/tasks/{task_id}/artifacts", timeout=60)
        assert response.status_code == 200
        assert isinstance(response.json(), list)
    ...
    def test_get_agent_task_step(self, url):
        ...
        response = requests.get(f"{url}/ap/v1/agent/tasks/{task_id}/steps/{step_id}", timeout=60)
        assert response.status_code == 200
